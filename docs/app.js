...
const direction = value => Number.isFinite(value) ? (value > 0 ? 'up' : value < 0 ? 'down' : 'neutral') : 'neutral';
...
function sparkline(market) {
  ...
  return `<svg viewBox="0 0 180 32" preserveAspectRatio="none" role="img" aria-label="近一个月日线收盘走势"><polyline points="${points}" fill="none" stroke="${values.at(-1)>values[0]?'#e13949':values.at(-1)<values[0]?'#008b65':'#75839a'}" stroke-width="2.5"/></svg>`;
}

function renderMarkets() {
  $('markets').innerHTML = snapshot.markets.map(m => {
    const available = m.status === 'ok', change = m.change_pct, trend = m.five_day_pct;
    return `<article class="quote quote-${available?direction(change):'neutral'}"><div class="quote-head"><strong><a href="${escapeText(safeUrl(m.url))}" target="_blank" rel="noopener noreferrer">${escapeText(m.name)} ↗</a></strong><small>${escapeText(m.unit)}</small></div>
      <div class="quote-price ${available?direction(change):'neutral'}">${available?Number(m.price).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}):'—'}</div>
      <div class="quote-change ${change>0?'up':change<0?'down':'neutral'}">${available && Number.isFinite(change)?`${change>0?'▲ 上涨':change<0?'▼ 下跌':'— 持平'} ${change>0?'+':''}${change.toFixed(2)}% 较前收盘`:'行情暂不可得'}</div>
      <div class="chart-label">近一个月走势 · 涨红跌绿</div>${sparkline(m)}<div class="quote-time">报价 ${timeText(m.quoted_at)} · 延迟数据</div>
      <div class="trend ${direction(trend)}" title="${escapeText(m.note||'')}">${Number.isFinite(trend)?`近 5 交易日 ${trend>=0?'+':''}${trend.toFixed(2)}%`:'历史数据不足'} · ${escapeText(m.source)}</div></article>`;
  }).join('');
}

// Transparent title-based attention rules: identify topics, never infer returns.
function newsSignal(title) {
  if (/违约|破产|制裁|袭击|空袭|战争|停火|封锁/.test(title))
    return {kind:'risk', label:'⚠ 风险事件', impact:'关注：风险偏好、供应链与避险需求', reason:'标题涉及信用或地缘风险；资产影响方向待核实'};
  if (/美联储|央行|国务院|财政部|证监会|降息|加息|降准|CPI|PPI|PMI|非农|通胀|GDP|LPR/.test(title))
    return {kind:'macro', label:'◎ 宏观焦点', impact:'关注：政策、流动性与资产估值', reason:'标题涉及宏观指标或政策；不代表已确认利好或利空'};
  if (/暴涨|暴跌|大涨|大跌|涨超|跌超|涨停|跌停|新高|新低|跳水/.test(title))
    return {kind:'move', label:'⚡ 市场异动', impact:'关注：相关资产波动与持续性', reason:'原文标题描述价格异动；后续趋势待核实'};
  return null;
}
...
function renderNews() {
  ...
  $('news').innerHTML = items.slice(0,limit).map((n,i) => { const signal=newsSignal(n.title); return `<article class="news-row ${signal?'attention attention-'+signal.kind:''}"><span class="rank">${i+1}</span><div>${signal?`<div class="signal-label ${signal.kind}" title="${escapeText(signal.reason)}">${signal.label}</div>`:''}<a class="news-title" href="${escapeText(safeUrl(n.url))}" target="_blank" rel="noopener noreferrer">${escapeText(n.title)}</a><div class="news-meta"><span>${escapeText(n.source)}</span><time datetime="${escapeText(n.published_at)}">${timeText(n.published_at)}</time><span class="news-category">${escapeText(n.category)}</span><span>原文 ↗</span></div>${signal?`<div class="impact-note">${signal.impact}<span>影响方向待核实</span></div>`:''}${n.summary?`<p class="news-summary">${escapeText(n.summary)}</p>`:''}</div></article>`; }).join('') || '<div class="empty">没有符合条件的近期新闻。可以更换分类或搜索词。</div>';
